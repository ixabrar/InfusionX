 ...
 * 
 * This script initializes your Firebase database with all participants
 * 
 * Steps:
 * 1. Complete Firebase setup (see FIREBASE_SETUP.md)
 * 2. Update src/config/firebase.js with your Firebase config
 * 3. Ensure Firebase rules allow writes (see FIREBASE_SETUP.md)
 * 4. Run: npm run dev
 * 5. Open http://localhost:5173/admin
 * 6. Go to DevTools Console (F12)
 * 7. Copy the entire blindCodingData and promptverseData arrays below
 * 8. Paste into console and run
 */

// BLIND CODING PARTICIPANTS (30 total)
...
// PROMPTVERSE PARTICIPANTS (49 total)
...
async function initializeFirebaseData() {
  try {
    ...
    const { initializeEvent } = await import('./services/firebaseService.js');
    
    await initializeEvent('blindcoding', blindCodingData);
    console.log('✅ Blind Coding participants loaded:', blindCodingData.length);
    
    await initializeEvent('promptverse', promptverseData);
    console.log('✅ Promptverse participants loaded:', promptverseData.length);
    
    console.log('✅ Firebase database initialized successfully!');
    location.reload();
  } catch (error) {
    console.error('❌ Error initializing Firebase:', error);
    console.log('Make sure you have completed the Firebase setup!');
  }
}
